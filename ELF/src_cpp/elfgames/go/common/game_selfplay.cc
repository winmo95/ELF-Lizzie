...
Coord GoGameSelfPlay::mcts_update_info(MCTSGoAI* mcts_go_ai, Coord c) {
  ...
  // Check the ranking of selected move. D4 Q16 R14 R6 info move Q16 visits 465 winrate 5112 order 3 pv Q16 D4 C17 D17 C16 D15 B14 info 
...
void GoGameSelfPlay::act() {
  ...
  if (_human_player != nullptr) {
    do {
	  ...
	  //_state_ext.forward(72);
	 // _state_ext.forward(73);
	unsigned  int count_ =0;
	  for(Coord c : undo_p){
		  if(count_ < undo_p.size()-1){
		  _state_ext.forward(c);
		  count_++;
		  }
	  }
        }
...
for(int i = 0;i<1;i++) curr_ai->act(s, &c);
