...
class SearchTreeT {
 public:
  ...
  MCTSResult makeAdvance(MCTSResult& lastResult,int i) {
    Node* root = getRootNode();
    MCTSResult result;
      switch(i){
         case 1 :root = getNode(lastResult.best_edge_info.child_node);break;
         case 2 :root = getNode(lastResult.two_edge_info.child_node);break;
         case 3 :root = getNode(lastResult.thr_edge_info.child_node);break;
         case 4 :root = getNode(lastResult.for_edge_info.child_node);break;
         case 5 :root = getNode(lastResult.five_edge_info.child_node);break;
         case 6 :root = getNode(lastResult.six_edge_info.child_node);break;
         case 7 :root = getNode(lastResult.sev_edge_info.child_node);break;
         case 8 :root = getNode(lastResult.ei_edge_info.child_node);break;
         case 9 :root = getNode(lastResult.ni_edge_info.child_node);break;
         case 10 :root = getNode(lastResult.ten_edge_info.child_node);break;
       }
  ...
  /*MCTSResult makeAdvance(const Action& action) {
    Node* r = getRootNode();
    Node* root;
    MCTSResult result;
    for (const auto& p : r->getStateActions()) {
      if (p.first == action) {
	  root = getNode(p.second.child_node);
         result.action_rank_method = MCTSResult::MOST_VISITED;
         result.addActions(root->getStateActions());
        //r = makeAction(p.second.child_node);
       } 
    }
      return result;
  }*/
