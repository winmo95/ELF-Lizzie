...
class TreeSearchT {
 public:
  ...
  MCTSResult halfrun(const State& root_state) {
    setRootNodeState(root_state);
 ...
 MCTSResult makeAdvance(MCTSResult& lastResult) {
    //searchTree_.makeAdvance(action);
	
    return searchTree_.makeAdvance(lastResult);
  }
// const Action& action

 std::string testinfo(Stone player_, MCTSResult& lastResult, int play_count ) {
    std::stringstream ss;
    MCTSResult lastResult_;
    MCTSResult lastResult2_;
    lastResult_= lastResult; 
// first
    
     ss << "info move "<< coord2str2(lastResult.best_action) << " visits " << lastResult.best_edge_info.num_visits << " winrate " <<    lastResult.best_edge_info.get_winrate(player_)        
       << " order " << play_count << " pv " << coord2str2(lastResult.best_action) << " ";

    for (; ; ) {
      lastResult2_ = searchTree_.makeAdvance(lastResult_,1);
      if(lastResult_.best_action == lastResult2_.best_action) break;
      if(lastResult2_.best_edge_info.num_visits < 50) break;
      ss << coord2str2(lastResult2_.best_action) << " ";
      lastResult_ = lastResult2_;
    }
    //ss << std::endl;

// second
     lastResult2_ = searchTree_.makeAdvance(lastResult,2);
  
   if(lastResult2_.best_edge_info.num_visits < 10){}
   else{
ss << "info move "<< coord2str2(lastResult.two_action) << " visits " << lastResult.two_edge_info.num_visits << " winrate " <<    lastResult.two_edge_info.get_winrate(player_)        
       << " order " << play_count << " pv " << coord2str2(lastResult.two_action) << " "; 
	ss<< coord2str2(lastResult2_.best_action) << " " ;
	lastResult_ = lastResult2_;
   	for (; ; ) {
      	lastResult2_ = searchTree_.makeAdvance(lastResult_,1);
...
// thr
	if(lastResult.thr_edge_info.num_visits < 10){}
	else{
		lastResult2_ = searchTree_.makeAdvance(lastResult,3);
		ss << "info move "<< coord2str2(lastResult.thr_action) << " visits " << lastResult.thr_edge_info.num_visits << " winrate " <<    lastResult.thr_edge_info.get_winrate(player_)        
       << " order " << play_count << " pv " << coord2str2(lastResult.thr_action) << " ";   

   		if(lastResult2_.best_edge_info.num_visits==0){}
   		else{
			ss<< coord2str2(lastResult2_.best_action) << " " ;
			lastResult_ = lastResult2_;
   			for (; ; ) {
...
// four
	if(lastResult.for_edge_info.num_visits < 10){}
	else{
		lastResult2_ = searchTree_.makeAdvance(lastResult,4);
		ss << "info move "<< coord2str2(lastResult.for_action) << " visits " << lastResult.for_edge_info.num_visits << " winrate " <<    lastResult.for_edge_info.get_winrate(player_)        
       << " order " << play_count << " pv " << coord2str2(lastResult.for_action) << " ";   
...
// five
	if(lastResult.five_edge_info.num_visits < 10){}
	else{
		lastResult2_ = searchTree_.makeAdvance(lastResult,5);
		ss << "info move "<< coord2str2(lastResult.five_action) << " visits " << lastResult.five_edge_info.num_visits << " winrate " <<    lastResult.five_edge_info.get_winrate(player_)        
       << " order " << play_count << " pv "<< coord2str2(lastResult.five_action) << " ";   
	  ...
	  //six
		...
		if(lastResult.six_edge_info.num_visits < 10){}
		else{
			lastResult2_ = searchTree_.makeAdvance(lastResult_,6);
			ss << "info move "<< coord2str2(lastResult.six_action) << " visits " << lastResult.six_edge_info.num_visits << " winrate " <<    lastResult.six_edge_info.get_winrate(player_)<< " order " << play_count << " pv "<< coord2str2(lastResult.six_action) << " ";   
        ...
        //sev
			if(lastResult.sev_edge_info.num_visits < 10){}
			else{
				lastResult2_ = searchTree_.makeAdvance(lastResult,7);
				ss << "info move "<< coord2str2(lastResult.sev_action) << " visits " << lastResult.sev_edge_info.num_visits << " winrate " <<    lastResult.sev_edge_info.get_winrate(player_)<< " order " << play_count << " pv "<< coord2str2(lastResult.sev_action) << " ";   
				...
				//eight
				if(lastResult.ei_edge_info.num_visits < 10){}
				else{
					lastResult2_ = searchTree_.makeAdvance(lastResult,8);
					ss << "info move "<< coord2str2(lastResult.ei_action) << " visits " << lastResult.ei_edge_info.num_visits << " winrate " <<    lastResult.ei_edge_info.get_winrate(player_)        << " order " << play_count << " pv "<< coord2str2(lastResult.ei_action) << " ";   
					...
					//nine
					if(lastResult.ni_edge_info.num_visits < 10){}
					else{
						lastResult2_ = searchTree_.makeAdvance(lastResult,9);
						ss << "info move "<< coord2str2(lastResult.ni_action) << " visits " << lastResult.ni_edge_info.num_visits << " winrate " <<    lastResult.ni_edge_info.get_winrate(player_)  << " order " << play_count << " pv "<< coord2str2(lastResult.ni_action) << " ";   
						...
						//ten
						if(lastResult.ten_edge_info.num_visits < 10){}
						else{
							lastResult2_ = searchTree_.makeAdvance(lastResult,10);
							ss << "info move "<< coord2str2(lastResult.ten_action) << " visits " << lastResult.ten_edge_info.num_visits << " winrate " <<    lastResult.ten_edge_info.get_winrate(player_)        << " order " << play_count << " pv "<< coord2str2(lastResult.ten_action) << " ";   
