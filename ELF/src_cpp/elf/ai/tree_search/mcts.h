...
class MCTSAI_T : public AI_T<typename Actor::State, typename Actor::Action> {
 public:
  ...
  bool act(const State& s, Action* a) override {
	...
	output = ts_->testinfo(s.nextPlayer(),lastResult_,s.getPly());
	std::cout<<output;    
      clock.record("MCTS"); 
      } else {
      lastResult_ = ts_->run(s);
     }    
  /*  logger_->info("\n virtual_loss {}\n reward  {} \n visits  {}\n score {} \n",lastResult_.best_edge_info.blackeval, lastResult_.best_edge_info.reward, lastResult_.best_edge_info.num_visits, lastResult_.best_edge_info.get_winrate(s.nextPlayer()));
    */
    //printf("%s",ts_->testinfo(s.nextPlayer(),lastResult_,s.getPly()));
    //std::cout<< output;
 ...
 private:
  ...
  MCTSResult lastResult3_;
  MCTSResult lastResult4_;
  MCTSResult lastResult5_;
  MCTSResult lastResult6_;
  MCTSResult lastResult7_;
  MCTSResult lastResult8_;
  MCTSResult lastResult9_;
  MCTSResult lastResult10_;
  MCTSResult lastResult11_;
  MCTSResult lastResult12_;
